...
import { ShoppingCart, Trash2, Plus, Minus, CheckCircle, ChevronRight, MapPin } from 'lucide-react'
      ...
      <main className="min-h-screen bg-brand-cream flex items-center justify-center px-4 pt-16">
        ...
        >
          ...
          <p className="text-brand-text/60 mb-2">
            We've received your order and it's being prepared fresh.
          </p>
    ...
    <main className="min-h-screen bg-brand-cream">
      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        <div className="flex items-center justify-center gap-3 mb-10">
          {[{ n: 1, label: 'Cart' }, { n: 2, label: 'Details' }].map(({ n, label }) => (
            <div key={n} className="flex items-center gap-3">
              ...
              {n < 2 && <ChevronRight size={16} className="text-brand-dark/20" />}
            </div>
          ))}
        </div>
                ...
                <div className="grid md:grid-cols-[1fr_300px] gap-6">
                      ...
                      {items.map(item => (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {item.size && (
                              <p className="text-brand-text/40 text-xs mt-0.5">{item.size}</p>
                            )}
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            <button
                              ...
                              className="w-7 h-7 rounded-full border border-brand-dark/15 flex items-center justify-center hover:border-brand-primary transition-colors"
                              aria-label="Decrease"
                            >
                              <Minus size={12} />
                            </button>
                  ...
                  <div className="bg-white rounded-2xl p-6 shadow-sm border border-brand-dark/5 h-fit sticky top-24">
                    ...
                    <div className="space-y-2 text-sm mb-4">
                      ...
                      <div className="flex justify-between text-brand-text/60">
                        <span>Tax (8.25%)</span><span>${(total * 0.0825).toFixed(2)}</span>
                      </div>
                    </div>
          ...
          {step === 2 && (
            ...
            >
              <div className="space-y-5">
                ...
                {/* Name */}
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-wider text-brand-text/50 mb-1.5">
                    Name
                  </label>
                  <input
                    type="text"
                      ...
                      <button
                        ...
                        className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border text-sm text-left transition-all ${
                          location === loc
                            ...
                            : 'border-brand-dark/15 hover:border-brand-primary/40'
                        }`}
                      >
              ...
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-brand-dark/5 h-fit sticky top-24">
                ...
                {!location && (
                  <p className="text-xs text-center text-brand-text/40 mt-2">Select a location to continue</p>
                )}
                <button
                  onClick={() => setStep(1)}
                  className="w-full text-center text-sm text-brand-text/50 hover:text-brand-text mt-3 transition-colors"
                >
                  ← Back to cart
                </button>
              </div>
